...
//---------------------------------------------------------------------------
void D3DGraphics::GetFrameRect(RECT &aRect)
{
  IDirect3DSurface9* pBBuffer = NULL;
  D3DLOCKED_RECT rect;
  //
  pDevice->GetBackBuffer(0,0,D3DBACKBUFFER_TYPE_MONO,&pBBuffer);
  pBBuffer->LockRect(&rect,NULL,NULL);
  //
  D3DSURFACE_DESC desc;
  pBBuffer->GetDesc(&desc);
  //
  pBBuffer->UnlockRect();
  //
  aRect.bottom=desc.Height;
  aRect.top=0;
  aRect.left=0;
  aRect.right=desc.Width;
}
//---------------------------------------------------------------------------
